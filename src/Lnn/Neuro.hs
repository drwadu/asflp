...
showw lnn@(Lnn nn _ _) (And xs _ _) = stringifyAnd (map (\x -> maybe (show x) (showw lnn) (Seq.lookup x nn)) xs)
showw lnn@(Lnn nn _ _) (Or xs _ _) = stringifyOr (map (\x -> maybe (show x) (showw lnn) (Seq.lookup x nn)) xs)
...
update (Not x l u) l' u' = Not x l' u'
update (And xs l u) l' u' = And xs l' u'
update (Or xs l u) l' u' = Or xs l' u'
